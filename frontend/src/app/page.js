...
export default function MenuPage() {
  const { 
    ...
    cart, addToCart, removeFromCart, clearCart // Pull in cart logic
  ...
  const handleAddToCart = (item) => {
    ...
    setQuantities(prev => ({ ...prev, [item.id]: 1 })); // Reset UI quantity to 1
  };

  // --- NEW: Process Full Cart Checkout ---
  const handleCheckout = async () => {
    if (cart.length === 0) return;
    setIsCheckingOut(true);

    try {
      // Map the cart array exactly how your NestJS backend expects it
  ...
  return (
    <div className="py-8 pb-32"> {/* Added pb-32 so the floating cart doesn't cover items */}
      ...
      {/* --- NEW: Floating Cart Panel --- */}
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-4 md:p-6 z-50 animate-fade-in-up">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
              ...
              <button 
                ...
                className="px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-lg rounded-xl transition-colors shadow-lg disabled:bg-gray-400"
              >
